...
def _setupSSHDImpl(public_key, tunnel, ngrok_token, ngrok_region, mount_gdrive_to, mount_gdrive_from, is_VNC):
  ...
  if tunnel == "ngrok":
    ...
    cfd_proc = "!cloudflared tunnel --url tcp://localhost:5901 --metrics localhost:49589"
    time.sleep(4)
    if cfd_proc != None:
      raise RuntimeError("Failed to run cloudflared. Return code: See clouldflared.log for more info.")
    hostname = None
...
def setupVNC(ngrok_region = None, check_gpu_available = True, tunnel = None, mount_gdrive_to = None, mount_gdrive_from = None, public_key = None):
  stat, msg =  _setupVNC()
  if stat:
    msg += _setupSSHDMain(public_key, tunnel, ngrok_region, check_gpu_available, mount_gdrive_to, mount_gdrive_from, True)
